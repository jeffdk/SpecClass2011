...
namespace Observers {
  ...
  PointwiseAvg::PointwiseAvg(const std::string &Opts,
		    ...
		    const std::string& FirstColumnInDatFiles) const
  {
	...
	const int component_size = i->Size();
	const double * pdata = i->Data();
	for(int j=0; j<component_size; ++j) sum += *pdata++;
	count += component_size;
      }
    }
    sum = MPreduceAdd(sum,D.Communicator());
    count = MPreduceAdd(count,D.Communicator());
